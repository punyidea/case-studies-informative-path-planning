...
class RectangleInterpolator():
    ...
    def __init__(self, nx, ny, P0, P1, fem_data, time_dependent=False, verbose=False):
        ...
        # Now, we enlarge the mesh by one cell in every direction
        self.enx = self.nx + 2
        self.eny = self.ny + 2
    ...
    def pre_computations_mat(self, u):
        ...
        for j in range(ny + 1):  # Pythonics for j=0...ny
            for i in range(nx + 1):  # Pythonics for i=0...nx
                ...
                # If my query point is outside the original mesh, set u to zero
                if x > self.x1+1e-12 or x < self.x0-1e-12 or y < self.y0-1e-12 or y > self.y1+1e-12:
                    u_ij = 0
                else:
                    u_ij = u([x, y])
                # u_ij = nodal_vals[i + j * (nx + 1)]
                # print(u_ij-u([self.x0+i*self.hx, self.y0 + j*self.hy]))
                # self.U[i, j] = u_ij # used to get the scipy interpolator, for debug
